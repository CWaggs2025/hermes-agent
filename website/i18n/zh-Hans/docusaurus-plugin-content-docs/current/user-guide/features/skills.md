...
路径支持 `~` 展开和 `${VAR}` 环境变量替换。
`external_scan_timeout_seconds` 控制内置 skill 同步所使用的外部目录扫描安全边界。默认值为 10 秒，并限制在 1–10 秒之间。
...
- **网关物化快照是只读的**：网关使用经过验证的 profile 本地外部 skill 副本。`skill_manage` 无法修改这些派生快照文件；请从前台或单独受监督的进程修改权威外部目录，然后重新对账生成快照。
- **网关快照路径绝不向外导出**：外部 skill 的文本和关联文件仍可通过 `skill_view` 读取，但物化快照会禁用绝对目录提示、`${HERMES_SKILL_DIR}` 展开、内联 shell 预处理、直接脚本执行以及 sandbox 挂载/上传。后端 API 只保留普通路径，无法携带安全的快照租约，因此 Hermes 会安全拒绝，而不会交出验证后可能被替换的缓存路径。
- **本地优先**：如果同一 skill 名称同时存在于本地目录和外部目录中，本地版本优先。
- **集成式发现**：外部 skills 出现在系统提示词索引、`skills_list`、`skill_view` 以及 `/skill-name` 斜杠命令中。前台外部 skills 在其他方面与本地 skills 相同；网关物化快照仍受上述路径功能和 sandbox 导出限制。
- **不可用的路径不会阻塞网关就绪**：网关生命周期和状态命令不会扫描 skill 目录。网关运行时发现不会解析或遍历配置的外部目录，只读取经过验证的 profile 本地物化快照；没有有效快照时不加载外部 skill。网关启动时，本地同步尝试只会在适配器、cron、后台维护任务以及服务就绪信号都已运行后开始。若配置了外部目录，网关模式会把对账推迟到单独受监督的前台同步，并且绝不会启动外部扫描器。
- **目录扫描失败时安全降级**：前台或单独受监督的内置 skill 同步会在可终止的子进程中扫描并物化外部目录，不跟随目录符号链接，并以原子方式保留上一次成功的目录指针。过期的最后成功快照仍可供网关使用，但内置 skill 对账会推迟且不修改本地 skill。非空目录变为空目录需要两次独立的成功扫描确认。重复失败会采用有上限的退避策略。本地物化缓存最多保留四个完整版本及 1 GiB 的源内容副本；当前版本、紧邻的上一版本以及网关正在租用的版本绝不会被删除。若无法安全地为一个完整新版本预留空间，同步会在扫描或更改目录指针之前推迟。
- **必须使用无竞态的扫描方式**：外部目录对账目前要求支持基于目录描述符的相对遍历（受支持的 POSIX 运行环境可用）。无法提供该机制的平台（包括 Windows）会推迟这项可选对账，不会退回到可能在重命名竞态中跟随目录链接的路径遍历。网关对物化快照的读取目前使用仅适用于 Python 3.11 的描述符路径能力；Python 3.12/3.13 网关会安全拒绝并不加载外部快照，直到支持新版 pathlib 的遍历实现。网关启动和本地已安装的 skill 不受影响。
- **运行时发现会跳过不存在的路径**：在内置 skill 对账之外，不存在的配置目录会被忽略。永久不再使用的路径应从配置中移除；暂时不可用的共享挂载由上述对账保护机制处理。
